...
# ---------------- per-bounce diagnostics: spot maps ----------------
if len(res.spot_pattern) > 0:
    sp = res.spot_pattern
    th = np.linspace(0, 2 * np.pi, cfg.N, endpoint=False)
    mirror_centers = np.column_stack([cfg.R_ring * np.cos(th),
                                       cfg.R_ring * np.sin(th),
                                       np.zeros_like(th)])
    # bounce sequence (which mirror each hit corresponds to)
    bounce_mirror_idx = []
    k = 0
    for _ in range(len(sp)):
        bounce_mirror_idx.append(k)
        k = (k + cfg.chord_skip) % cfg.N
    bounce_mirror_idx = np.array(bounce_mirror_idx)

    # beam radius at each bounce: use the larger of tangential & sagittal
    w_per_bounce = np.maximum(prop["w_tangential"][1:len(sp) + 1],
                              prop["w_sagittal"][1:len(sp) + 1])

    # convert each hit to that mirror's LOCAL (tangential, sagittal) coords
    # tangential axis = -sin(theta), cos(theta), 0  (in-plane, tangent to ring)
    # sagittal axis   = 0, 0, 1                     (vertical)
    local_t = np.zeros(len(sp))
    local_s = np.zeros(len(sp))
    for i, hit in enumerate(sp):
        m_idx = bounce_mirror_idx[i]
        ang = th[m_idx]
        tan_axis = np.array([-np.sin(ang), np.cos(ang), 0.0])
        sag_axis = np.array([0.0, 0.0, 1.0])
        d = hit - mirror_centers[m_idx]
        local_t[i] = d @ tan_axis
        local_s[i] = d @ sag_axis

    # ---------- mirror map grid ----------
    st.subheader("Per-mirror spot map  —  where each bounce lands on each mirror")
    st.caption("Each subplot = one mirror's clear aperture (red circle). "
               "Hits are dots sized by the Gaussian beam radius at that bounce. "
               "If dots+circles approach the aperture edge → clipping risk.")
    from plotly.subplots import make_subplots
    cols = min(5, cfg.N)
    rows = int(np.ceil(cfg.N / cols))
    fig_mm = make_subplots(rows=rows, cols=cols,
                           subplot_titles=[f"M{i}" for i in range(cfg.N)],
                           horizontal_spacing=0.04, vertical_spacing=0.08)
    aper = cfg.mirror_aperture
    phi = np.linspace(0, 2*np.pi, 60)
    for m in range(cfg.N):
        r_idx = m // cols + 1
        c_idx = m % cols + 1
        # aperture circle
        fig_mm.add_trace(go.Scatter(x=aper*np.cos(phi), y=aper*np.sin(phi),
                                    mode="lines", line=dict(color="red", width=1.5),
                                    showlegend=False, hoverinfo="skip"),
                        row=r_idx, col=c_idx)
        # hits on this mirror
        mask = bounce_mirror_idx == m
        if mask.any():
            # marker size in pixels ~ beam_radius scaled
            sizes = np.clip(w_per_bounce[mask] / aper * 40, 6, 60)
            fig_mm.add_trace(go.Scatter(
                x=local_t[mask], y=local_s[mask],
                mode="markers",
                marker=dict(size=sizes, color=np.where(mask)[0],
                            colorscale="Viridis", opacity=0.65,
                            line=dict(color="black", width=0.5)),
                hovertemplate=("bounce %{customdata[0]}<br>"
                               "tan: %{x:.2f} mm<br>sag: %{y:.2f} mm<br>"
                               "w: %{customdata[1]:.2f} mm<extra></extra>"),
                customdata=np.column_stack([np.where(mask)[0] + 1,
                                            w_per_bounce[mask]]),
                showlegend=False),
                row=r_idx, col=c_idx)
        fig_mm.update_xaxes(range=[-aper*1.1, aper*1.1],
                            scaleanchor=f"y{m+1}" if m > 0 else "y",
                            row=r_idx, col=c_idx,
                            zeroline=True, zerolinecolor="lightgray")
        fig_mm.update_yaxes(range=[-aper*1.1, aper*1.1],
                            row=r_idx, col=c_idx,
                            zeroline=True, zerolinecolor="lightgray")
    fig_mm.update_layout(height=180 * rows + 80,
                         margin=dict(l=10, r=10, t=40, b=10),
                         showlegend=False)
    for a in fig_mm.layout.annotations:
        a.font.size = 11
    st.plotly_chart(fig_mm, use_container_width=True)

    # ---------- unfolded spot pattern ----------
    st.subheader("Unfolded spot pattern  —  azimuth vs z, marker size = beam radius")
    azimuth = np.degrees(np.arctan2(sp[:, 1], sp[:, 0]))
    fig_uf = go.Figure(go.Scatter(
        x=azimuth, y=sp[:, 2],
        mode="markers+lines",
        line=dict(color="lightgray", width=0.6),
        marker=dict(size=np.clip(w_per_bounce * 8, 4, 40),
                    color=np.arange(len(sp)), colorscale="Viridis",
                    showscale=True, colorbar=dict(title="bounce #"),
                    line=dict(color="black", width=0.4)),
        hovertemplate=("bounce %{customdata[0]}<br>"
                       "az: %{x:.1f}°<br>z: %{y:.2f} mm<br>"
                       "w: %{customdata[1]:.2f} mm<extra></extra>"),
        customdata=np.column_stack([np.arange(1, len(sp) + 1), w_per_bounce]),
    ))
    fig_uf.add_hline(y=cfg.H / 2, line=dict(color="red", dash="dash"),
                     annotation_text="+H/2 (cell top)")
    fig_uf.add_hline(y=-cfg.H / 2, line=dict(color="red", dash="dash"),
                     annotation_text="-H/2 (cell bottom)")
    fig_uf.update_layout(xaxis_title="azimuth [deg]", yaxis_title="z [mm]",
                         height=420, margin=dict(l=10, r=10, t=10, b=40))
    st.plotly_chart(fig_uf, use_container_width=True)

    # ---------- three orthogonal views ----------
    st.subheader("Orthogonal views")
    fig_ov = make_subplots(rows=1, cols=3,
                           subplot_titles=("Top  (x-y)", "Front  (x-z)", "Side  (y-z)"),
                           horizontal_spacing=0.08)
    # top
    fig_ov.add_trace(go.Scatter(x=sp[:, 0], y=sp[:, 1], mode="lines+markers",
                                line=dict(color="royalblue", width=1),
                                marker=dict(size=4, color=np.arange(len(sp)),
                                            colorscale="Viridis"),
                                showlegend=False), row=1, col=1)
    fig_ov.add_trace(go.Scatter(x=mirror_centers[:, 0], y=mirror_centers[:, 1],
                                mode="markers",
                                marker=dict(size=8, color="red", symbol="square"),
                                showlegend=False), row=1, col=1)
    # front
    fig_ov.add_trace(go.Scatter(x=sp[:, 0], y=sp[:, 2], mode="lines+markers",
                                line=dict(color="royalblue", width=1),
                                marker=dict(size=4, color=np.arange(len(sp)),
                                            colorscale="Viridis"),
                                showlegend=False), row=1, col=2)
    # side
    fig_ov.add_trace(go.Scatter(x=sp[:, 1], y=sp[:, 2], mode="lines+markers",
                                line=dict(color="royalblue", width=1),
                                marker=dict(size=4, color=np.arange(len(sp)),
                                            colorscale="Viridis"),
                                showlegend=False), row=1, col=3)
    fig_ov.update_xaxes(title_text="x [mm]", row=1, col=1)
    fig_ov.update_yaxes(title_text="y [mm]", scaleanchor="x", row=1, col=1)
    fig_ov.update_xaxes(title_text="x [mm]", row=1, col=2)
    fig_ov.update_yaxes(title_text="z [mm]", row=1, col=2)
    fig_ov.update_xaxes(title_text="y [mm]", row=1, col=3)
    fig_ov.update_yaxes(title_text="z [mm]", row=1, col=3)
    fig_ov.update_layout(height=380, margin=dict(l=10, r=10, t=40, b=40),
                         showlegend=False)
    st.plotly_chart(fig_ov, use_container_width=True)

    # ---------- per-mirror hit count ----------
    st.subheader("Per-mirror bounce count")
    counts = np.bincount(bounce_mirror_idx, minlength=cfg.N)
    fig_cnt = go.Figure(go.Bar(x=[f"M{i}" for i in range(cfg.N)],
                               y=counts, marker_color="seagreen",
                               text=counts, textposition="auto"))
    fig_cnt.update_layout(yaxis_title="bounces on this mirror",
