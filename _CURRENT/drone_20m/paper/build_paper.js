...
body.push(P("A 1.3 mm waist placed at the hole is badly mismatched to the cell eigenmode (mirror-plane radius w = √(M²λL/π sin θ) ≈ 0.3–0.45 mm here); the beam then breathes to ~3× its injected size, spots overlap, and the hole clips 13.5 % of the power at entry and again at exit. Instead, a small lens focuses the 1.3 mm collimated input so the in-cell waist (0.20–0.33 mm) sits near mid-chord: the beam rides the eigenmode at 0.21–0.42 mm for the entire path, the radius at the hole is ~0.3–0.37 mm, and the 1.3 mm hole transmits ≈100 % both ways. Cell transmission is then exactly T(R) = R^(n−1). At closure the round-trip ABCD map is the identity, so the exit beam reproduces the injected Gaussian and leaves through the same hole, separated from the input axis by 2θ_i (22.5° for N = 16), placing laser and detector side by side behind M0 with no beamsplitter."));
