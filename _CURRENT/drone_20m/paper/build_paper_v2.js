...
body.push(H2("6.7 Independent validation in a commercial ray tracer (COMSOL)"));
body.push(P("Beyond the open-source Optiland cross-check of §3, every menu design was independently rebuilt and traced in COMSOL Multiphysics 6.4 (Ray Optics Module) [28] — a commercial finite-element solver whose ray engine shares no code, no geometry representation and no numerical method with ours (analytic surfaces and Newton intersection here; a meshed boundary representation with element-facet intersection there). The models were generated programmatically from the same machine-readable design rows: each catalogue mirror is placed as an analytic spherical cap of the exact catalogue curvature at its ring coordinate, carries a specular wall, and the single chief ray is released on the verified launch line and traced through the full folded path. The COMSOL ray-position export is reduced to bounce vertices (intersection of successive chord segments) and differenced against our exact trace. Agreement is at the single-micrometre level for every design (Table 11, Fig. 17): the chief-ray RMS deviation over 98–228 bounces of 9–29 m folded path ranges from 1.3 to 8.7 µm, with a worst single-bounce deviation of 19.7 µm — four to five orders of magnitude below the ~1.6 mm beam and the 1.3 mm coupling hole, i.e. optically negligible, and consistent with the finite mesh faceting of the curved caps in the commercial model. COMSOL independently reproduces the re-entrance condition as well: the traced chief ray returns to within 9–18 µm of the entrance point at the designed exit bounce in every design, confirming the beam threads back out through the coupling hole. The two engines therefore agree not only on the path length and pass count but on the full three-dimensional spot pattern, bounce by bounce (Fig. 18). The integral quantity agrees more tightly still: the optical path length COMSOL accumulates over the complete folded trace differs from our exact tracer's by less than 5 µm over 9–29 m of path — 0.01–0.24 ppm (Table 11) — so the two engines agree on the quantity spectroscopy actually uses, the absorption path, to sub-parts-per-million precision."));
body.push(...tbl(
  "Independent COMSOL Ray-Optics cross-validation: chief-ray deviation from the exact tracer over the complete folded path, COMSOL's own re-entrance (exit-vertex distance from the entrance point), and the integrated optical-path-length difference.",
  ["design", "bounces", "RMS [µm]", "worst [µm]", "median [µm]", "re-exit [mm]", "ΔOPL [ppm]"],
  [
    ["ceiling 29.0 m", "204", "1.29", "3.11", "1.00", "0.010", "−0.02"],
    ["tri-gas 25.7 m", "176", "3.81", "9.09", "2.93", "0.009", "−0.08"],
    ["H2-opt. 23.8 m", "176", "3.60", "8.96", "2.25", "0.010", "+0.09"],
    ["27 m (Ø160)", "228", "4.24", "10.16", "2.67", "0.010", "−0.04"],
    ["compact 20.7 m", "204", "2.80", "6.88", "1.79", "0.009", "+0.09"],
    ["standard-tier 22.3 m", "182", "1.33", "2.94", "1.01", "0.010", "−0.05"],
    ["two-inch 19.0 m", "152", "8.66", "19.71", "5.68", "0.018", "−0.24"],
    ["sparse 15.3 m", "112", "3.26", "7.82", "2.19", "0.009", "+0.04"],
    ["mini 9.1 m (½″)", "98", "1.26", "3.01", "0.80", "0.008", "−0.01"],
  ],
  [2100, 950, 1000, 1000, 1100, 1100, 1100]));
body.push(...fig("comsol_agreement.png",
  "Independent COMSOL Ray-Optics cross-validation: chief-ray RMS and worst-bounce deviation from our exact tracer over the full folded path of each design (log scale). Every design agrees to below the 10 µm RMS Optiland-parity criterion; the residual scales with the commercial model's mesh faceting of the curved mirror caps.", 600));
body.push(...fig("comsol_raytrace_montage.png",
  "COMSOL Ray-Optics traces (top view) for three designs spanning the mirror-count range. The chord-skip pattern fills the disc through near-diameter chords and, in every case, clears the central coupling hole (white) that the beam threads on entry and exit — independently reproducing our re-entrance geometry.", 620));
body.push(P("The spot-pattern check validates where the beam goes; a second, independent COMSOL check validates how wide it is. A Gaussian beam's 1/e² envelope is exactly the envelope of a paraxial ray family released from its waist, and the transverse second moments of any ray bundle propagate through an optical system by the same ABCD matrices as the beam's complex q parameter — so a 17-ray bundle (a chief ray plus two eight-ray phase rings spanning the two transverse principal planes, released with the design's actual waist, waist offset and M²) carries the full astigmatic beam width through every reflection using only the already-validated ray engine, with no wave-optics solve. At each exported time step the bundle offsets are projected onto the plane normal to the chief direction and the principal 1/e² radii are recovered as the eigenvalues of the 2×2 moment matrix, then compared against the platform's astigmatic ABCD envelope at the same path length (steps adjacent to a mirror bounce are masked, since the bundle rays cross the mirror at slightly different times). The free-space control reproduces the textbook Gaussian w(z) law to numerical precision — band-edge deviation below 2×10⁻⁴ % over 0.6 m (Fig. 19). In-cell, the compact flight design's COMSOL width band over the full 20.2 m sampled path matches the ABCD prediction to 0.43 % at the lower band edge and 0.73 % at the upper (Fig. 20); the pointwise scatter that develops mid-path is lattice dephasing — the bundle's periodic breathing slipping phase against the model's as micrometre-level per-bounce differences accumulate over thousands of samples — with a signed mean deviation of +1.0 %, i.e. drift without bias. Sweeping the same harness over the entire menu (Table 12; Fig. 21 shows the tri-gas flagship, and a per-design panel ships with each design's data package) gives the architecture-level result: the ABCD width band is conservative for every design. The widest beam COMSOL ever records never exceeds the model's band maximum, with margins of 0.4–18 % — largest for the strongly breathing 27 m and half-inch designs, whose model band edges are set by extreme constellation excursions the real bundle undershoots. Since mirror apertures, hole clearances and the spot-overlap criterion are all sized against the model band, this direction of disagreement is the safe one: the commercial solver independently confirms the beam is never wider than the envelope the design rules assume."));
body.push(...tbl(
  "COMSOL ray-bundle beam-width validation across the menu: principal 1/e² width range recorded by COMSOL over the full in-cell path vs the astigmatic ABCD envelope, and the band-edge deviations (negative = COMSOL inside the model band; the upper edge is negative for every design, so the model band is conservative everywhere).",
  ["design", "path [m]", "COMSOL w [mm]", "ABCD w [mm]", "min-edge dev [%]", "max-edge dev [%]"],
  [
    ["ceiling 29.0 m", "28.5", "0.415–0.447", "0.415–0.449", "+0.01", "−0.40"],
    ["tri-gas 25.7 m", "25.3", "0.264–0.327", "0.265–0.337", "−0.43", "−3.05"],
    ["H2-opt. 23.8 m", "23.4", "0.230–0.456", "0.232–0.469", "−0.74", "−2.73"],
    ["27 m (Ø160)", "26.2", "0.164–0.267", "0.155–0.327", "+5.81", "−18.28"],
    ["compact 20.7 m", "20.2", "0.286–0.452", "0.287–0.455", "−0.43", "−0.73"],
    ["standard-tier 22.3 m", "22.0", "0.184–0.295", "0.192–0.321", "−4.31", "−8.31"],
    ["two-inch 19.0 m", "18.8", "0.173–0.376", "0.173–0.396", "+0.20", "−5.05"],
    ["sparse 15.3 m", "15.1", "0.198–0.287", "0.198–0.310", "+0.06", "−7.36"],
    ["mini 9.1 m (½″)", "8.9", "0.109–0.255", "0.110–0.308", "−1.38", "−17.15"],
  ],
  [2100, 950, 1450, 1450, 1300, 1300]));
body.push(...fig("comsol_beam_freespace.png",
  "Beam-width validation, free-space control: principal 1/e² radii of the 17-ray COMSOL bundle (dots, major/minor axes) against the Gaussian w(z) law (lines) over 0.6 m from the launch waist. The ray-bundle moment method reproduces the analytic envelope to below 2×10⁻⁴ % at both band edges, validating the harness before it is applied in-cell.", 600));
body.push(...fig("comsol_beam_D150_14cm_flight.png",
  "Beam-width validation in-cell, compact flight design (20.2 m sampled path, 2234 exported steps): COMSOL bundle principal radii (dots) against the astigmatic ABCD envelope (red, tangential/sagittal). The width band agrees to 0.43 %/0.73 % at its edges; the growing pointwise phase slip of the periodic breathing is lattice dephasing from accumulated micrometre-level bounce differences and carries no bias (signed mean +1.0 %).", 620));
body.push(...fig("comsol_beam_D190_26m_trigas.png",
  "Beam-width validation in-cell, tri-gas flagship (25.3 m sampled path): COMSOL bundle principal radii vs the astigmatic ABCD envelope. The COMSOL band sits inside the model band (−0.43 % / −3.05 % at the edges) — the pattern repeated across the full menu (Table 12): the model's width envelope, against which all apertures and clearances are sized, is conservative for every design.", 620));
body.push(H2("6.8 Operational physics: polarization, vibration, and remaining effects"));
...
const refs = [
  ...
  "COMSOL AB, COMSOL Multiphysics 6.4, Ray Optics Module, Stockholm, Sweden (2026).",
];
