...
body.push(H2("6.3 Construction tolerance: what actually builds this cell"));
body.push(P("Manufacturing processes were mapped onto the Monte-Carlo machinery by converting published process capabilities into tolerance grades — seat flatness over the 25.4 mm pocket into mirror tilt σ (CNC precision 0.1 mrad; CNC standard 0.5; printed body with machined seats 0.3; industrial SLA 1.5; MJF/SLS 2.5; FDM 4.0), positional accuracy into decentre, radial accuracy into ring radius — and running the exact-trace Monte-Carlo per process (Fig. 14). The verdict is structured: dense 204-chord designs survive only precision-CNC aluminium; the sparse-pattern designs (144 chords, and especially the 112-chord 15.3 m design with 7 spots per mirror) complete 100 % of paths even on standard CNC and on a printed body with machined seats, failing only the no-realignment exit criterion by ~20 µm of pattern walk — recovered by the one-time ring trim every build performs. A fully polymer-printed cell fails three independent ways: as-printed accuracy (best industrial SLA holds ±0.27 mm on a 180 mm part against the ±0.1 mm ring budget, and no printed surface holds 2.5–12.7 µm seat flatness), in-service stability (PA12's CTE shrinks the thermal closure window to ±2–6 K and its 0.2–0.3 % moisture swelling walks the ring radius ~100 µm with the weather), and dynamics (printed lids resonate at 330–670 Hz, inside the 60–700 Hz rotor band, where aluminium lids sit at 1.2–2.0 kHz). And the economics do not reward the compromise: a one-off CNC aluminium housing costs the same class as a good polymer print ($250–600 standard; $600–1200 precision), so the '$200 printed cell' is real only as the sparse-pattern design in a hybrid printed-body/machined-seat build for benchtop use — the flight-proven precedent for this cell class is likewise monolithic CNC aluminium on vibration dampers [19]. Printing is the right tool for the enclosure, window bezels and ducting, and a $50 print remains the correct fit-check before committing the machining."));
...
body.push(...tbl(
  "Housing mass and lid fundamental mode (parametric model; solid walls — pocketing removes ≈30 %).",
  ["design", "Al 6061 mass", "Mg AZ31B mass", "PA12 print mass", "lid f₁ Al", "lid f₁ PA12"],
  [
    ["standard 20.4 m (Ø180)", "1.71 kg", "1.26 kg", "0.89 kg", "1.24 kHz", "327 Hz"],
    ["compact 20.7 m (Ø141)", "1.23 kg", "0.92 kg", "0.66 kg", "2.03 kHz", "535 Hz"],
    ["flight max 29.0 m (Ø183)", "1.66 kg", "1.20 kg", "0.82 kg", "1.20 kHz", "317 Hz"],
  ],
  [2300, 1350, 1400, 1500, 1200, 1250]));
body.push(H2("6.4 Cell windows"));
...
body.push(H2("6.5 CAD, build plan and the active tier"));
body.push(P("Because the optical datums live in the design row, the housing CAD is generated from it: a parametric script emits STEP (for Fusion 360 / FEA) and STL models — ring body with mirror pockets on the exact ring radius, the M0 beam cone at the design incidence angle, window boss, gas ports and bolt circle — for four housings from Ø129 to Ø183 (Fig. 15). The assembly procedure is: measure the delivered mirror curvatures; machine the ring to the interpolated radius from the linear trim law (Table 8); bond or screw the mirrors (bonded optics qualify to 20–40 g shock, an order of magnitude above the ~1.5–3 g, 60–700 Hz multirotor environment); align once with the collimator focus ring and launch adjuster; and isolate the whole cell from the airframe on rubber dampers — rigid inside, damped outside, the configuration validated in drone flight by [19]. For the active-alignment tier the same parts close a control loop: the Monte-Carlo shows these designs complete their paths even at flight tolerances and need recovery only of the coherent pattern walk (~1–3 mm), for which the ring heater is the slow actuator (1.2–1.7 µm of ring radius per kelvin, millikelvin-limited resolution), a ±5 mrad launch piezo the fast one, and a quadrant detector behind the coupling hole the error signal — roughly US$400–700 of additional hardware buys the step from 29 to 38.6 m (uniform) or 51.7 m (two-SKU)."));
...
body.push(H2("6.6 Operational physics: polarization, vibration, and remaining effects"));
