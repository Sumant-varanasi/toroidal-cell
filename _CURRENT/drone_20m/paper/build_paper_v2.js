...
body.push(H2("6.7 Independent validation: a commercial-solver protocol"));
body.push(P("Beyond the Optiland cross-check of §3, the verification chain includes a ready-to-run validation package for a commercial ray-tracing environment (COMSOL Ray Optics), structured so a reviewer or collaborator can reproduce every claim without our code: per-design geometry files carrying each mirror's centre, optical-axis normal, sagittal axis, curvature and clear aperture plus the exact launch ray in the same coordinates; the perturbation envelopes of §6.3 for the parametric sweeps; and a native reference run log — nominal plus every coherent single-axis worst case (all mirrors at +max, then −max, per degree of freedom, and each launch axis at its box edge; 37 runs per design) — with pass counts, path lengths, and edge, hole and exit clearances to reproduce, plus a comparison harness that scores any exported chief-ray trajectory against the exact trace with the same sub-10 µm criterion the Optiland validation met at 0.000 µm RMS. The worst-case sweeps confirm the drift budget's structure: every menu design passes eight of ten mirror axes at both architecture envelopes, and the recurring exceptions are the coherent radial pair (the ring-radius/thermal mode of §6.3, governed by trim and temperature) and, for the two boundary designs only, whole-ring vertical displacement at the hybrid envelope — consistent with their known walk-tightness."));
